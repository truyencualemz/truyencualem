...
function buildNav() {
  ...
  [
    ...
    const active = App.view===v||(v==='library'&&['add-comic','edit-comic'].includes(App.view))||(v==='chapters'&&CHAP_VIEWS.includes(App.view));
  ...
  const role = window.CURRENT_ROLE || 'admin';
...
function buildTopbar() {
  const T = { library:'Thư viện','add-comic':'Thêm truyện mới','edit-comic':'Sửa thông tin truyện',chapters:'Chương / Tập','add-chapter':'Thêm chương tranh','edit-chapter':'Sửa chương tranh','add-text-chapter':'Thêm chương chữ','edit-text-chapter':'Sửa chương chữ',settings:'Cài đặt' };
  const S = { library:'Quản lý truyện song ngữ','add-comic':'Điền thông tin truyện','edit-comic':'Chỉnh sửa thông tin',chapters:'Danh sách chương','add-chapter':'Upload trang ảnh / PDF','edit-chapter':'Chỉnh sửa trang','add-text-chapter':'Soạn nội dung đoạn văn','edit-text-chapter':'Chỉnh sửa đoạn văn',settings:'Cấu hình' };
...
function buildContent() {
  ...
  switch (App.view) {
    ...
    case 'edit-comic':         c.appendChild(Admin.viewEditComic());   break;
    ...
    case 'users':      if(window.CURRENT_ROLE==='admin')Admin.viewUsers(c);      else c.innerHTML='<div style="padding:40px;text-align:center;color:#555">Không có quyền.</div>'; break;
    case 'settings':   if(window.CURRENT_ROLE==='admin')Admin.viewSettings(c);   else c.innerHTML='<div style="padding:40px;text-align:center;color:#555">Không có quyền.</div>'; break;
  }
}
